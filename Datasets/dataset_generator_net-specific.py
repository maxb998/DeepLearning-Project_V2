...
class Params:
    ...
    grids_cell_areas_sqrt:torch.Tensor
    ...
    def set_netin(self):
        ...
        self.grids_cell_areas_sqrt = self.grids_cell_size_px * 2
...
def argParser() -> Params:
    ...
    parser.add_argument('--objSizeLimits', metavar='[MAX_OBJ_SIZE,MIN_OBJ_SIZE]', type=restricted_int, nargs=2, default=[150,16], help='Limits in size of tank and flag resize after render in pixels(not exactly the max and min size)')
    ...
    parser.add_argument('--maxOverlap', metavar='MAX_OVERLAP', type=float, default=0.7, help='The maximum amount an object can be overlapped by another. Value range: (0,1)')
    ...
    p.nimgs = args.nimgs
    p.maxtanks = args.maxObjs[0]
    p.maxflags = args.maxObjs[1]
    p.imgWidth = args.imgShape[0]
    p.imgHeight = args.imgShape[1]
    p.maxObjSize = args.objSizeLimits[0]
    p.minObjSize = args.objSizeLimits[1]
...
def create_renderer(p:Params) -> o3d.visualization.rendering.OffscreenRenderer:
    ...
    renderer.scene.set_lighting(renderer.scene.LightingProfile.SOFT_SHADOWS, (0.577, -0.577, -0.577))
...
def create_material_record(obj_class:int) -> o3d.visualization.rendering.MaterialRecord:
    ...
    mtl.base_clearcoat = 1
    mtl.base_roughness = 0.5
    mtl.base_clearcoat_roughness = 0.3
    mtl.base_anisotropy = 1
...
def render_obj(renderer:o3d.visualization.rendering.OffscreenRenderer, rescaled_size:int) -> np.ndarray:
    ...
    # rendered_img = cv2.GaussianBlur(rendered_img, (5,5), 9)
...
def cmp_box_intersection_w_areas(newbox:np.ndarray, oldboxes:np.ndarray, maxAreaOverlap:float) -> tuple[np.ndarray, np.ndarray]:
    x21, y21, x22, y22 = oldboxes[:,0], oldboxes[:,1], oldboxes[:,2], oldboxes[:,3]
...
def is_cell_occupied(p:Params, newbox:torch.Tensor, coords_list:list[int]) -> bool:
    ...
    bbox_area_sqrt = torch.sqrt((newbox[2] - newbox[0]) * (newbox[3] - newbox[1]))
    best_grid_id = torch.abs(bbox_area_sqrt - p.grids_cell_areas_sqrt).argmin()
...
def add_single_element(p:Params, renderer:o3d.visualization.rendering.OffscreenRenderer, img:np.ndarray, obj_class:int, previous_elems:np.ndarray, coords_list:list[int], iternum:int, box_size:int) -> bool:
    ...
    rnd_gen_offsets = np.array([p.imgWidth, p.imgHeight], dtype=np.float64) - (box_size+1)
...
def get_background(p:Params, backgrounds:list[str]) -> np.ndarray:

    if np.random.randint(0,40) == 0: # image is random noise with 2.5% chance
        return np.random.randint(0, 255, (p.imgHeight,p.imgWidth,3), dtype=np.uint8)

    fname = backgrounds[np.random.randint(len(backgrounds))]
    ...
    if option == 0:
        return cv2.resize(img, (p.imgHeight, p.imgWidth))
    
    if option == 1: # resize image in a bit of a random way
        new_h, new_w = np.random.randint(min(p.imgHeight, img.shape[0]), max(p.imgHeight, img.shape[0])), np.random.randint(min(p.imgWidth, img.shape[1]), max((p.imgWidth, img.shape[1])))
        img = cv2.resize(img, (new_h, new_w))

    if img.shape[0] < p.imgHeight or img.shape[1] < p.imgWidth:
        img = cv2.resize(img, (max(img.shape[0], p.imgHeight), max(img.shape[1], p.imgWidth)))
    
    # no need to check if option == 2 -> get random piece of the original image of size(p.imgHeight, p.imgWidth)
    start_h, start_w = int(0), int(0)
    high_h, high_w = img.shape[0] - p.imgHeight, img.shape[1] - p.imgWidth
    if high_h > 1:
        start_h = np.random.randint(0, high_h)
    if high_w > 1:
        start_w = np.random.randint(0, high_w)

    return img[start_h:start_h+p.imgHeight, start_w:start_w+p.imgWidth]
...
def generate_dataset(p:Params):
    ...
    renderCount = int(0)
    while i < n:
        img = get_background(p, backgrounds)
        
        assert img.shape[0] == p.imgHeight and img.shape[1] == p.imgWidth
        
        # need to decide how many tanks and flags to add based on the size of the objects themself or viceversa
        if p.maxtanks > 0:
            ntanks = np.random.randint(low=1, high=p.maxtanks)
        if p.maxflags > 0:
            nflags = np.random.randint(low=1, high=p.maxflags)
        
        renderCount += ntanks + nflags

        if renderCount + ntanks + nflags < 131041: # critical value after which open3d starts not working correctly
